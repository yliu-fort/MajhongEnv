...
class MahjongEnvKivyWrapper:
    ...
    def _draw_melds(
        ...
        if player_idx >= len(melds):
            return
        num_kan = sum([True if m["type"]=="kan" else False for m in melds[player_idx]] + [False])
        margin_side = 14
        max_meld_width = meld_tile[0] * 4 + 12
        x = max(margin_side, play_rect.width - margin_side - max_meld_width) + hand_tile[0]*max(num_kan - 2, 0) + margin_side
