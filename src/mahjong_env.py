...
class MahjongEnvCNNEncoding(MahjongEnvBase):
    """
    一个简化的麻将环境示例。
    """
    def __init__(self, num_players=4):
        super(MahjongEnvCNNEncoding, self).__init__(num_players=num_players)

        # 定义动作空间
        # 维度1: 0 ~ 13: 切出14张手牌中的哪一张
        # 维度2: 0, 1: 取消/确认
        self.action_space = spaces.MultiDiscrete([136, 2])
        ...
        #)
        self.observation_space = spaces.Box(low = 0, high = 1, shape=(4, 34, 34), dtype=np.float32) # C * H * W

        self.reset()
    
    def get_observation(self, player):
        """根据当前玩家，返回相应的状态表示。"""
        obs = np.zeros((4, 34, 34), dtype=np.float32)
        plist = [(player + i + 3) % 4 for i in range(4)] # kamicha, player, shimocha, toimen
        p_width = 5
        # layer 0 - 4*p_width-1: player hands, player open melds, player discarded pile
        for j, p in enumerate(plist):
            voffset = j*p_width
            #0 layer 1: player hands
            if p == player:
                obs[:, :, voffset] = self.tiles_136_to_4x34(self.hands[p])
            #1-4 layer 2: player open melds
            for i, m in enumerate(self.melds[p]):
                obs[:, :, i+1+voffset] = self.tiles_136_to_4x34(m['m'])
            #5 layer 3: player discarded pile
            obs[:, :, 5+voffset] = self.tiles_bool_to_4x34(self.discard_pile[p])
        # layer 20: dora indicator
        voffset = 4*p_width # 4*5=20
        obs[:, :, voffset] = self.tiles_136_to_4x34(self.dora_indicator)
        # layer 21: last discarded tile
        voffset = 4*p_width+1
        obs[:, :, voffset] = self.tiles_136_to_4x34([self.last_discarded_tile])
        # layer 22: oya, player wind, round wind, sc
        voffset = 4*p_width+2
        obs[:, self.get_distance(player, self.oya), voffset] = 1
        # layer 23-24: player wind, round wind
        obs[:, 27 + player, voffset+1] = 1
        obs[:, 27 + (self.round[0] // 4) % 4, voffset+2] = 1
        # layer 25-28 scores
        for j, p in enumerate(plist):
            obs[:, :, voffset+3+j] = self.scores[p] / 1000.0
        # layer 29-32: riichi
        for j, p in enumerate(plist):
            obs[:, :, voffset+7+j] = self.riichi[p]
        # layer 33: phase
        obs[:, MahjongEnvBase.PHASE_MAP[self.phase], 33] = 1
        ...
        match self.phase:
            case "discard":
                if self.riichi[self.current_player]:
                    # 立直后只能打最后摸到的牌
                    return self.tiles_136_to_bool([self.hands[self.current_player][-1]]) + [False, False]
                else:
                    return self.tiles_136_to_bool(self.hands[self.current_player]) + [False, False]
            ...
            case "pon"|"kan"|"chi":
                ...
                if self.is_selecting_tiles_for_claim:
                    ...
                    tiles = [self.hands[self.current_player][i] for i, is_valid in enumerate(possible_mask) if is_valid]
                    return self.tiles_136_to_bool(tiles) + [False, False]
                else:
                    return [False] * 136 + [True, True]
            ...
            case "ankan":
                ...
                tiles = [self.hands[self.current_player][i] for i, is_valid in enumerate(mask) if is_valid]

                return self.tiles_136_to_bool(tiles) + [True, True]
            ...
            case "riichi":
                ...
                tiles = [self.hands[self.current_player][i] for i, s in enumerate(shantens) if s <= 0]
                return self.tiles_136_to_bool(tiles) + [True, True]

        return [False]*138
    
    def action_map(self, action_grp):
        raw_action, confirm = action_grp
        try:
            action = self.hands[self.current_player].index(raw_action)
        except ValueError:
            action = 255 # 这是一个无效索引，可以帮助我们在后续处理中发现错误
...
class MahjongEnv1p(MahjongEnvCNNEncoding):
    """
    一个简化的麻将环境示例。
    """
    def __init__(self, num_players=4):
        super(MahjongEnv1p, self).__init__(num_players=num_players)

        self.opponent_agent = RandomDiscardAgent(self)
        self.position = randint(0,3) # 玩家的位置
    
    def step(self, action_grp): 
        obs, reward, done, info =  super().step(action_grp)

        while (not self.done) and (self.current_player != self.position):
            opponent_action_grp = self.opponent_agent.predict(obs)
            obs, _, done, info =  super().step(opponent_action_grp)
        
        # 顺位奖励
        if self.done:
            # 更新顺位
            reward_rank = [1.0,0,0.0,-1.0]
            rank = info["rank"][self.position]
            reward += reward_rank[rank]
