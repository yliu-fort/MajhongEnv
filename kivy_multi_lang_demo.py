...
def register_multilingual_font() -> str:
    """Register a font that can render the demo's multilingual text."""

    font_path = resource_find("data/fonts/DejaVuSans.ttf")
    if font_path:
        LabelBase.register(
            name="LanguageDemoFont",
            fn_regular=font_path,
            fn_bold=font_path,
            fn_italic=font_path,
            fn_bolditalic=font_path,
        )
        return "LanguageDemoFont"

    # Fallback to the default Kivy font. It will still work for Latin
    # characters even if DejaVuSans cannot be resolved at runtime.
    return "Roboto"


UI_FONT_NAME = register_multilingual_font()


class LanguageSpinnerOption(SpinnerOption):
    """Spinner option that ensures translated text renders correctly."""
...
<LanguageSwitcher>:
    ...
    BoxLayout:
        ...
        Spinner:
            ...
            option_cls: "LanguageSpinnerOption"
...
class LanguageSwitcher(BoxLayout):
    ...
    ui_font_name = StringProperty(UI_FONT_NAME)
