...
class TestDataloaderWithRiichiDatasetZarr(unittest.TestCase):
    def test_dataloader_with_riichi_dataset_zarr(self):
        tmp_path = 'tmp'
        root = os.path.join(tmp_path, "zarr_ds3")
        ...
        x0, y0, m0 = batches[0]
        self.assertEqual(x0.shape, (2, NUM_FEATURES, NUM_TILES, NUM_TILES))
        self.assertEqual(torch.equal(y0, torch.tensor([1, 2])), True)
        self.assertEqual(m0.shape, (2, NUM_TILES))

        x1, y1, m1 = batches[1]
        self.assertEqual(x1.shape, (1, NUM_FEATURES, NUM_TILES, NUM_TILES))
        self.assertEqual(y1.item(), 3)
        self.assertEqual(m1.shape, (1, NUM_TILES))
